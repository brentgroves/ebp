...
function init(dispatch){
  sql1Cnt=0;
  dispatch({ type:GRACTION.LOG_END_FAILED, failed:false });
  dispatch({ type:GRACTION.LOG_END_DONE, done:false });
}
...
function execSQL1(disp,getSt){
  ...
  if ('development'==process.env.NODE_ENV) {
    console.log(`SQLLogEnd.execSQL1() top=>${sql1Cnt}`);
  }
  ...
  var connection = new sql.Connection(CONNECT.cribDefTO, function(err) {
    ...
    if(null==err){
      if ('development'==process.env.NODE_ENV) {
        console.log(`SQLLogEnd.execSQL1() Connection Sucess`);
      }
      let sproc;
      ...
      request.execute(sproc, function(err, recordsets, returnValue) {
        ...
        if(null==err){
          // ... error checks
          if ('development'==process.env.NODE_ENV) {
            console.log(`SQLLogEnd.execSQL1() Sucess`);
          }
        }else {
          if(++sql1Cnt<ATTEMPTS) {
            if ('development'==process.env.NODE_ENV) {
              console.log(`SQLLogEnd.execSQL1().query:  ${err.message}` );
              console.log(`sql1Cnt = ${sql1Cnt}`);
            }
          }else{
            if ('development'==process.env.NODE_ENV) {
              console.log(`SQLLogEnd.execSQL1():  ${err.message}` );
            }
            dispatch({ type:GRACTION.SET_REASON, reason:err.message });
            dispatch({ type:GRACTION.SET_STATE, state:GRSTATE.FAILURE });
            dispatch({ type:GRACTION.LOG_END_FAILED, failed:true });
          }
        }
      });
      dispatch({ type:GRACTION.LOG_END_DONE, done:true });
    }else{
      if(++sql1Cnt<ATTEMPTS) {
        if ('development'==process.env.NODE_ENV) {
          console.log(`SQLLogEnd.Connection: ${err.message}` );
          console.log(`sql1Cnt = ${sql1Cnt}`);
        }
      }else{
        if ('development'==process.env.NODE_ENV) {
  ...
  connection.on('error', function(err) {
    if(++sql1Cnt<ATTEMPTS) {
      if ('development'==process.env.NODE_ENV) {
        console.log(`SQLLogEnd.connection.on(error): ${err.message}` );
        console.log(`sql1Cnt = ${sql1Cnt}`);
      }
